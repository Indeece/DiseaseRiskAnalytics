...
class SGDLogisticRegression(RegressorMixin):
        ...
        while (current_step < self.max_steps and continue_flag):
            ...
            for i in range(0, L, self.batch_size):
                ...
                if current_step != self.max_steps:
                    ...
                    pred = [1/(1 + np.e**np.dot(self.W, X_batch[x])) for x in range(len(X_batch))]
                    # calculate the gradient
                    grad = sum([np.dot(sum(X_batch[x]), (Y_batch - pred)[x]) for x in range(len(X_batch))])
                    # move the weight vector towards anti-gradient
                    self.W -= (self.lr * grad)

                    loss_history.append(mean_absolute_error(pred, Y_batch))
                    if current_step%100 == 0:
                        print(f"Training is finished on {current_step/100}% with accuracy {loss_history[current_step]}")
                        # print(self.W)
    ...
    def predict(self, X):
        ...
        return [1/(1 + np.e**np.dot(self.W, x_scaled[x])) for x in range(len(x_scaled))]
...
LogicRegressor.fit(data_train, Y_train)   # train our logistic Regression
# print(LogicRegressor.predict(data_test))
print(mean_absolute_error(LogicRegressor.predict(data_test), Y_test))  # see the metrics
